...
import {
  ...
  Lightning, CaretLeft, Play, Pause,
  PencilSimple, FileText, ArrowsClockwise, PaperPlaneTilt, Robot,
  MusicNote, VideoCamera, ArrowLineDown,
...
const mockHistory = [
  { id: 'h1', title: 'meeting-recording.mp3', status: 'completed' as const, time: '2026-06-15 14:30', size: '12.3 MB', result: '会议讨论了Q4产品规划，确定了三个主要方向……', resultId: 'result-speech-to-text-1' },
  { id: 'h2', title: 'interview-zhang.mp3', status: 'completed' as const, time: '2026-06-12 09:15', size: '8.7 MB', result: '受访者张总介绍了公司数字化转型的三个阶段……', resultId: 'result-speech-to-text-2' },
  { id: 'h3', title: 'lecture-ai.mp3', status: 'completed' as const, time: '2026-06-10 16:00', size: '22.1 MB', result: '大家好，欢迎来到AI基础入门课程……', resultId: 'result-speech-to-text-3' },
]
      ...
      <div className="max-w-[1440px] mx-auto px-[34px] pt-9 pb-16 flex flex-col gap-6">
        ...
        <div className="grid grid-cols-1 md:grid-cols-[3fr_1fr] gap-6 items-stretch">
          ...
          <div className="rounded-[16px] border border-[#f0f2f8] bg-white overflow-hidden" style={CARD_SHADOW} ref={scrollRef}>
            <div className="p-6 h-full flex flex-col">
              <h3 className="text-[15px] font-medium text-foreground mb-3 shrink-0">生成设置</h3>
              ...
              {file && (
                <div className="flex flex-col items-center pt-4 mt-auto shrink-0">
                  <Button onClick={handleGenerate} disabled={isProcessing} className="h-[46px] px-12 rounded-[10px] bg-[#4f55ec] hover:bg-[#4f55ec]/80 text-white font-medium text-[15px] min-w-[200px]" style={{ boxShadow: '0px 12px 28px rgba(87,92,233,0.22)' }}>
                    {isProcessing ? (<span className="flex items-center gap-2"><SpinnerGap className="h-4 w-4 animate-spin" />正在生成...</span>) : (<span className="flex items-center gap-2"><Lightning className="h-4 w-4" weight="fill" />立即生成</span>)}
                  </Button>
        ...
        {resultText && !isProcessing && (
          <div className="rounded-[18px] border border-[#f0f2f8] overflow-hidden" style={{ ...CARD_SHADOW, background: 'rgba(255,255,255,0.96)' }} ref={resultRef}>
            <div className="px-6 pt-6 pb-2" style={{ background: 'linear-gradient(180deg, #f8faff 0%, #ffffff 100%)' }}>
              <div className="flex items-center justify-between"><h3 className="text-[18px] font-medium text-[#3f4558]">生成结果</h3></div>
            </div>
        ...
        <div className="rounded-[18px] border border-[#f0f2f8] bg-white overflow-hidden" style={CARD_SHADOW}>
          ...
          {history.length === 0 ? (
            ...
            <div className="px-6 pb-6">
              ...
              {history.map(item => (
                <div key={item.id} className="px-4 py-3 flex items-center gap-4 hover:bg-[#fbfcff] transition-colors bg-white">
                  <div className="w-8 h-8 rounded-lg bg-sky-50 flex items-center justify-center shrink-0"><FileAudio className="h-4 w-4 text-sky-500" weight="fill" /></div>
                  ...
                  <div className="flex items-center gap-1 ml-auto shrink-0">
                    <Button variant="outline" size="sm" className="h-[34px] px-[14px] rounded-[8px] text-[13px] font-normal gap-1.5 border-[#e2e6f3] bg-white text-[#596176] hover:bg-[#f3f5ff] hover:border-[#dfe3ff] hover:text-[#596176] shadow-none" onClick={() => onViewResult?.(item.resultId, item.title)}><ArrowLineDown className="h-3.5 w-3.5" />下载</Button>
                    <Button variant="ghost" size="sm" className="h-[34px] px-2 text-[13px] text-[#9ca2b5] hover:text-destructive gap-1" onClick={() => handleDeleteHistory(item.id)}><Trash className="h-3.5 w-3.5" />删除</Button>
                  </div>
                </div>
              ))}
              </div>
